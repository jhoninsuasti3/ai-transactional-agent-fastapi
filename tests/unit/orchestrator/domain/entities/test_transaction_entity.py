...
class TestTransactionEntity:
    ...
    def test_create_valid_transaction(self):
        ...
        txn = Transaction(
            user_id="user-123", recipient_phone="3001234567", amount=50000, currency=Currency.COP
        )
    ...
    def test_phone_validation_10_digits(self):
        ...
        txn = Transaction(user_id="user-123", recipient_phone="3001234567", amount=50000)
        assert txn.recipient_phone == "3001234567"

        # Invalid - less than 10 digits
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="300123456", amount=50000)
    ...
    def test_phone_validation_starts_with_3(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="2001234567", amount=50000)
    ...
    def test_phone_validation_only_digits(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="300ABC4567", amount=50000)
    ...
    def test_phone_validation_cleans_separators(self):
        ...
        txn = Transaction(user_id="user-123", recipient_phone="300-123-4567", amount=50000)
        assert txn.recipient_phone == "3001234567"

        txn2 = Transaction(user_id="user-123", recipient_phone="(300) 123-4567", amount=50000)
    ...
    def test_amount_validation_greater_than_zero(self):
        ...
        # Invalid - zero
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="3001234567", amount=0)
        assert "greater than 0" in str(exc_info.value) or "greater than zero" in str(exc_info.value)

        # Invalid - negative
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="3001234567", amount=-1000)

    def test_amount_minimum_1000_cop(self):
        """Test amount must be at least 1000 COP."""
        # Valid - at minimum
        txn = Transaction(user_id="user-123", recipient_phone="3001234567", amount=1000)
        assert txn.amount == 1000

        # Invalid - below minimum
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="3001234567", amount=999)
        assert "minimum 1000" in str(exc_info.value)

    def test_amount_maximum_5000000_cop(self):
        """Test amount cannot exceed 5,000,000 COP."""
        # Valid - at maximum
        txn = Transaction(user_id="user-123", recipient_phone="3001234567", amount=5_000_000)
        assert txn.amount == 5_000_000

        # Invalid - above maximum
        with pytest.raises(ValidationError) as exc_info:
            Transaction(user_id="user-123", recipient_phone="3001234567", amount=5_000_001)
        assert "exceeds limit" in str(exc_info.value)

    def test_amount_rounded_to_2_decimals(self):
        """Test amount is rounded to 2 decimal places."""
        txn = Transaction(user_id="user-123", recipient_phone="3001234567", amount=50000.999)
    ...
    def test_transaction_method_chaining(self):
        ...
        txn2 = Transaction(user_id="user-123", recipient_phone="3001234567", amount=50000)
